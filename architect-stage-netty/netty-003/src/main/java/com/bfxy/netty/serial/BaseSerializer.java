...
public abstract class BaseSerializer {
	 ...
	 * @param bytes	读取的数组
	 */
	public BaseSerializer readFromBytes(byte[] bytes) {
		readBuffer = Unpooled.copiedBuffer(bytes);
		read();
		readBuffer.clear();
		return this;
	}
	...
	public <I> I read(Class<I> clz) {
		...
		if ( clz == int.class || clz == Integer.class) {
			...
			t = readString();
		} else if (BaseSerializer.class.isAssignableFrom(clz)){
			try {
				byte hasObject = this.readBuffer.readByte();
				if(hasObject == 1){
					BaseSerializer temp = (BaseSerializer)clz.newInstance();
	...
	public BaseSerializer writeByte(Byte value) {
		writeBuffer.writeByte(value);
		return this;
	}

	public BaseSerializer writeShort(Short value) {
		writeBuffer.writeShort(value);
		return this;
	}

	public BaseSerializer writeInt(Integer value) {
		writeBuffer.writeInt(value);
		return this;
	}

	public BaseSerializer writeLong(Long value) {
		writeBuffer.writeLong(value);
		return this;
	}

	public BaseSerializer writeFloat(Float value) {
		writeBuffer.writeFloat(value);
		return this;
	}

	public BaseSerializer writeDouble(Double value) {
		writeBuffer.writeDouble(value);
		return this;
	}

	public <T> BaseSerializer writeList(List<T> list) {
		if (isEmpty(list)) {
	...
	public <K,V> BaseSerializer writeMap(Map<K, V> map) {
		if (isEmpty(map)) {
			writeBuffer.writeShort((short) 0);
			return this;
		}
	...
	public BaseSerializer writeString(String value) {
	...
	public BaseSerializer writeObject(Object object) {
		...
		if(object == null){
			...
			if (object instanceof BaseSerializer) {
				writeByte((byte)1);
				BaseSerializer value = (BaseSerializer) object;
